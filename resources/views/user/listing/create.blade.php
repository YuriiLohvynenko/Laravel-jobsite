...
<head>
	...
	<script src="{{ asset('js/taostr.js') }}"></script>
	<script src="{{ asset('js/toastrscript.js') }}"></script>
    <script src="{{ asset('js/photoswipe.min.js') }}"></script>
    <script src="{{ asset('js/photoswipe-ui-default.min.js') }}"></script>
	<script src="{{ asset('plugins/taostr.js') }}"></script>
	<script src="{{ asset('plugins/toastrscript.js') }}"></script>
...
<div class="tabs margin-bottom-30">
    ...
    <form method="post" id="listing-post-form" class="listing-post-form">
        ...
        <div class="tabs-content tabs-no-space">
                ...
                <div class="tab tab4" data-tab-id="4">
                    <!-- Client Jobs Active (client-active) -->
                    <div class="row">
                        <div class="col-12 ow-container" style="background-color: #f9f9f9;background: #f9f9f9;">
                            <div class="ow-inner-container">
                    ...
                    <section class="section-container">
                        ...
                        <div class="row margin-top-50">
                            <div class="col-12">
                                <a href="javascript:;" onclick="submitForm(); return false;" class="button button-sliding-icon ripple-effect pull-right" id="submit">@lang('core.postListing') <i class="icon-material-outline-arrow-right-alt"></i></a>
                            </div>
                        </div>
                    </section></div>
                </div>
...
//    }
    ...
    function submitForm() {
		var datepick = $('#date_time').val();
		var total = $('#totalBudget').html();
		var data = $(".listing-post-form").serialize();
		// console.log(data);
		if(datepick && total != '$0'){
		  // alert(total);
        $.easyAjax({
            ...
            success: function (response) {
                ...
                if(response.status == 'success') {
					// var data = response.url.split('?');
					// var id = parseInt(atob(data[1]));
					var id = response.listingid;
                    $('#dropZoneListing').val(id);
					toastr.success('List creating successfully', {timeOut: 5000});
					// $(".tab4").html('<div class="loader"></div>');
					$("#spin").html('<div class="blockUI blockMsg blockElement" style="z-index: 1000010; position: absolute; padding: 0px; margin: 0px; width: 30%; top: 10%; left: 442px; text-align: center; color: rgb(0, 0, 0); border: 0px; cursor: wait;"><div class="loading-message"><div class="block-spinner-bar"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div></div></div>');
			...
			// funcBtns.alertError3();
			toastr.error('Please increse your budget',{timeOut: 5000});
		}
        return false;
    }
