    ...
    $(function() {
        ...
        @endphp
            ...
            var file = {
                ...
                size: '{{ getRemoteFilesize(asset('../storage/app/public/listing-files/'.$file->file_name .'.'.$file->file_format)) }}'
            };
            myDropzone.options.addedfile.call(myDropzone, file);
            @if($file->file_format != 'pdf')
                myDropzone.options.thumbnail.call(myDropzone, file, '{{ asset('../storage/app/public/listing-files/'.$file->file_name .'.'.$file->file_format) }}');
            @endif
            @endforeach
    });
</script>
        ...
        $.easyAjax({
            ...
            success: function (response) {
                ...
                if(response.status == 'success') {
                    $('#dropZoneListing').val(response.returnId);
