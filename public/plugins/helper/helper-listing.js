(function($) {
    ...
    $.easyAjax = function(options) {
        ...
        function showResponseMessage(msg, type, toastrOptions) {
            ...
            if (opt.messagePosition == "toastr") {
                ...
                if (ele.length == 0) {
                    var eleClass = $(opt.container).find(".custom-input");

                    if (eleClass.length == 0) {
                        $(opt.container).find(".custom-input")
                            .before('<div id="alert">' + html + "</div>");
                    }else{
                        $(opt.container).find(".submit-field")
                            .before('<div id="alert">' + html + "</div>");
                    }
            ...
            success: function (response) {
                ...
                if (response.status == "success") {
                    ...
                    if (opt.removeElements == true) {
                        var eleClass = $(opt.container).find(".custom-input");
                        if (eleClass.length != 0) {
                            $(opt.container).find(".custom-input, button, input").remove();
                        }else{
            ...
            if (typeof response.errors != "undefined") {
                ...
                if (opt.errorPosition == "field") {
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        var eleClass = $(opt.container).find(".custom-input");
                        var eleClassTwo = $(opt.container).find(".submit-field");

                        if (eleClass.length != 0 && key != 'category' && key != 'term_condition'  && key != 'job_location' ) {
                            var grp = ele.closest(".custom-input");
                        }else{
                       ...
                       // var helpBlockContainer = $(grp).find("div:first");
                        if($(ele).is(':radio')){
                           var helpBlockContainer = $(grp).find("div:eq(2)");
                       }
    ...
    $.showErrors = function(object) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            $(grp).find(".help-block").remove();
            //var helpBlockContainer = $(grp).find("div");
