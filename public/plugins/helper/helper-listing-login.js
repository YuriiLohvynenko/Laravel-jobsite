(function($) {
    'use strict';
    $.easyAjaxLogin = function(options) {
        var defaults = {
            type: 'GET',
            container: 'body',
            blockUI: true,
        ...
        if (typeof opt.beforeSend != "function") {
            opt.beforeSend = function() {
                ...
                if (opt.blockUI) {
                    $.easyBlockUILogin(opt.container);
                }

                if (opt.disableButton) {
                    loadingButtonLogin(opt.buttonSelector);
                }
            }
        }
        ...
        if (typeof opt.complete != "function") {
            opt.complete = function (jqXHR, textStatus) {
                if (opt.blockUI) {
                    $.easyUnblockUILogin(opt.container);
                }

                if (opt.disableButton) {
                    unloadingButtonLogin(opt.buttonSelector)
                }
            }
        }
            ...
            opt.error = function(jqXHR, textStatus, errorThrown) {
                try {
                    ...
                    if (typeof response == "object") {
                        ...
                        showResponseMessageLogin(msg, "error");
                    }
                }
                catch (e) {
        ...
        function showResponseMessageLogin(msg, type, toastrOptions) {
            var typeClasses = {
                "error": "danger",
                "success": "success",
                "primary": "primary",
            ...
            if (opt.messagePosition == "toastr") {
                $.showToastrLogin(msg, type, toastrOptions);
            }
            else {
                var ele = $(opt.container).find("#alert");
                var html = '<div class="alert alert-'+ typeClasses[type] +'">' + msg +'</div>';
                if (ele.length == 0) {
                    var eleClass = $(opt.container).find(".input-with-icon-left");

                    if (eleClass.length == 0) {
                        $(opt.container).find(".input-with-icon-left:first")
                            .before('<div id="alert">' + html + "</div>");
                    }else{
                        $(opt.container).find(".submit-field:first")
                            .before('<div id="alert">' + html + "</div>");
                    }
                ...
                if (response.status == "success") {
                    if (response.action == "redirect") {
                        if (opt.redirect) {
                            ...
                            showResponseMessageLogin(message, "success", {
                                timeOut: 100000,
                        ...
                        if (typeof response.message != "undefined") {
                            showResponseMessageLogin(response.message, "success");
                        }
                    }
                    ...
                    if (opt.removeElements == true) {
                        var eleClass = $(opt.container).find(".input-with-icon-left");
                        if (eleClass.length != 0) {
                            $(opt.container).find(".input-with-icon-left, button, input").remove();
                        }else{
                            $(opt.container).find(".submit-field, button, input").remove();
                        }
        ...
        function handleFailLogin(response) {
            if (typeof response.message != "undefined") {
                showResponseMessageLogin(response.message, "error");
            }
            if (typeof response.errors != "undefined") {
                ...
                if (opt.errorPosition == "field") {
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        var eleClass = $(opt.container).find(".input-with-icon-left");
                        if (eleClass.length != 0) {
                            var grp = ele.closest(".input-with-icon-left");
                        }else{
                            var grp = ele.closest(".submit-field");
                        }
                ...
                $('.tabs').each(function() {
                    ...
                    // Animate Tab Height
                    function animateTabHeightLogin() {
        ...
        function loadingButtonLogin(selector) {
        ...
        function unloadingButtonLogin(selector) {
            var button = $(opt.container).find(selector);
    ...
    $.easyBlockUILogin = function (container, message) {
    ...
    $.easyUnblockUILogin = function (container) {
    ...
    $.showToastrLogin = function(toastrMessage, toastrType, options) {
    ...
    $.ajaxModalLogin = function(selector, url, onLoad) {
    ...
    $.showErrorsLogin = function(object) {
